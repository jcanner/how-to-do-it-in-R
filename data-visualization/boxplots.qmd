...
Notice the use of the native pipe `|>` to connect the two functions from `ggformula`. Another version of a pipe in R is `%>%`. Both will work, it is just that `%>%` relies on another package and `|>` is built into base R.  
...
### Other Modifications <a class="btn btn-primary" data-bs-toggle="collapse" href="#other-modifications" role="button" aria-expanded="false" style="h4" aria-controls="other-modifications">Show</a> {.unnumbered .unlisted}
...
Modify the values for `height = ` argument and see how they affect the point distribution on the y-axis. Remember, the y-axis has no meaning here, so the heights of the points are random and not representative of any element of the data. Just the position on the x-axis is meaningful. 
...
We can add a color to fill the boxes by telling R to fill the boxes with a specified color either using a [built in color from R](https://r-charts.com/colors/) or using a [hex code for colors](https://htmlcolorcodes.com/color-picker/) .
